...
const DOMParser$1 = getDOMParserImplementation();
...
Strophe.Request = class Request {
  ...
  getResponse() {
    ...
    if (this.xhr.responseXML && this.xhr.responseXML.documentElement) {
      ...
      node = new DOMParser$1().parseFromString(this.xhr.responseText, 'application/xml').documentElement;
...
class XMLConverter {
  constructor(options = {}, bin = '') {
    ...
    this.bin = bin instanceof ArrayBuffer ? new Uint8Array(bin) : new TextEncoder().encode(bin); // Class variable to store binary data
    this.acc = new Uint8Array(0); // Initialize acc as a class variable
  }
  setBin(bin) {
    this.bin = bin instanceof ArrayBuffer ? new Uint8Array(bin) : new TextEncoder().encode(bin);
  }
  ...
  decodeString() {
    ...
    const swappedBin = this.bin.slice(0, length);
    this.bin = this.bin.slice(length);
    const bin = this.swapEnabled ? XMLConverter.swapAdjacentBits(swappedBin) : swappedBin;
    const decoder = new TextDecoder('utf-8');
    return decoder.decode(bin);
  }
  static swapAdjacentBits(bin) {
    const swapped = new Uint8Array(bin.length);
    for (let i = 0; i < bin.length; i++) {
      const charCode = bin[i];
      let swappedCharCode = 0;
      for (let bit = 0; bit < 8; bit += 2) {
        const bit1 = charCode >> bit & 1;
        const bit2 = charCode >> bit + 1 & 1;
        swappedCharCode |= bit1 << bit + 1 | bit2 << bit;
      }
      swapped[i] = swappedCharCode;
    }
    return swapped;
  }

  // static encodeNumber(number) {
  //     if (number === 0) return new Uint8Array([0]); // Use null character for 0
  //     const result = [];
  //     while (number > 0) {
  //         let byte = number & 0x7f; // Get the last 7 bits
  ...
  encodeNumberAddtoAcc(number) {
    if (number === 0) this.addToAcc(new Uint8Array([0])); // Use null character for 0
    const result = [];
    while (number > 0) {
      let byte = number & 0x7f; // Get the last 7 bits
      number >>= 7; // Shift right by 7 bits
    ...
    this.addToAcc(new Uint8Array(result));
  }
  decodeNumber() {
    if (this.bin[0] === 0) {
      this.bin = this.bin.slice(1);
      return 0; // Handle null character for 0
    }
    let number = 0;
    let shift = 0;
    let i = 0;
    ...
    this.bin = this.bin.slice(j + 1);
    return number;
  }
  addToAcc(array) {
    const newAcc = new Uint8Array(this.acc.length + array.length);
    newAcc.set(this.acc, 0);
    newAcc.set(array, this.acc.length);
    this.acc = newAcc;
  }
  encodeElement(element, opts = {}) {
    // this.acc = new Uint8Array(0); // Reset acc for each element

    this.encodeStringAddToAcc(element.name);
    this.encodeNumberAddtoAcc(element.attrs.length);
    element.attrs.forEach(([key, value]) => {
      this.encodeStringAddToAcc(key);
      this.encodeStringAddToAcc(value);
    });
    if (element.children.length === 0) {
      this.encodeNumberAddtoAcc(IS_ELEMENT);
      this.encodeNumberAddtoAcc(0); // number of children
    } else {
      const childrenType = element.children[0].name ? IS_ELEMENT : IS_CONTENT;
      this.encodeNumberAddtoAcc(childrenType);
      if (childrenType === IS_ELEMENT) {
        this.encodeNumberAddtoAcc(element.children.length); //number of children
      }
      element.children.forEach(child => {
        if (child.name) {
          this.encodeElement(child, opts);
        } else if (child.content) {
          this.encodeStringAddToAcc(child.content);
        }
      });
    }
    return this.acc;
  ...
  decodeElement(opts = {}) {
    ...
    const children = [];
    if (childrenType === IS_ELEMENT) {
      childCount = this.decodeNumber();
      for (let i = 0; i < childCount; i++) {
        const child = this.decodeElement(opts);
        children.push(child);
      }
  ...
  elementToString(element) {
    const attrs = element.attrs.map(([key, value]) => `${key}="${value}"`).join(' ');
    const children = element.children.map(child => {
      if (typeof child === 'object' && child.name) {
        return this.elementToString(child);
      } else if (typeof child === 'object' && child.content) {
        return child.content;
      }
      return '';
    }).join('');
    return `<${element.name}${attrs ? ' ' + attrs : ''}>${children}</${element.name}>`;
  }
}

// Function to convert a DOM element to a JavaScript object
function domToJSObject(domElement) {
  const name = domElement.nodeName;
  const attrs = Array.from(domElement.attributes).map(attr => [attr.name, attr.value]);
  const children = Array.from(domElement.childNodes).map(child => {
    if (child.nodeType === 1) {
      // Element node
      return domToJSObject(child);
...
const encodeXmlString = xmlString => {
  const converter = new XMLConverter(opts);
  const domParser = new DOMParser();
  const domDocument = domParser.parseFromString(xmlString, 'application/xml');
  const jsObject = domToJSObject(domDocument.documentElement);
  const uint8Arr = converter.encodeElement(jsObject, opts);
  // converter.setBin(uint8Arr.buffer);
  // const decodedElement = converter.decodeElement(opts);
  // const decodedXmlString = converter.elementToString(decodedElement);
  return uint8Arr.buffer;
};

// encodeXmlString(xmlString);

const decodeBinaryData = async blob => {
  const arrayBuffer = await blob.arrayBuffer();
  const converter = new XMLConverter(opts, arrayBuffer);
  const decodedElement = converter.decodeElement(opts);
  const decodedXmlString = converter.elementToString(decodedElement);
...
Strophe.Websocket = class Websocket {
  ...
  _onInitialMessage(message) {
    decodeBinaryData(message.data).then(xmlString => {
      if (xmlString.indexOf('<open ') === 0 || xmlString.indexOf('<?xml') === 0) {
        // Strip the XML Declaration, if there is one
        const data = xmlString.replace(/^(<\?.*?\?>\s*)*/, '');
        if (data === '') return;
        const streamStart = new DOMParser$1().parseFromString(data, 'text/xml').documentElement;
        this._conn.xmlInput(streamStart);
        this._conn.rawInput(xmlString);
      ...
      } else if (xmlString.indexOf('<close ') === 0) {
        // <close xmlns="urn:ietf:params:xml:ns:xmpp-framing />
        // Parse the raw string to an XML element
        const parsedMessage = new DOMParser$1().parseFromString(xmlString, 'text/xml').documentElement;
        ...
        const string = this._streamWrap(xmlString);
        const elem = new DOMParser$1().parseFromString(string, 'text/xml').documentElement;
        this._conn._connect_cb(elem, null, xmlString);
      }
    }).catch(error => {
      console.error(error);
    });
  ...
  _disconnect(pres) {
    if (this.socket && this.socket.readyState !== WebSocket.CLOSED) {
      ...
      try {
        const binaryData = encodeXmlString(closeString);
    ...
    if (data.length > 0 && !this._conn.paused) {
      for (let i = 0; i < data.length; i++) {
        if (data[i] !== null) {
          ...
          const binaryData = encodeXmlString(rawStanza);
          this.socket.send(binaryData);
        }
      }
      this._conn._data = [];
  ...
  _onMessage(message) {
    decodeBinaryData(message.data).then(xmlString => {
      ...
      if (xmlString === close) {
        ...
        return;
      } else if (xmlString.search('<open ') === 0) {
        // This handles stream restarts
        elem = new DOMParser$1().parseFromString(xmlString, 'text/xml').documentElement;
        if (!this._handleStreamStart(elem)) {
          return;
        }
      } else {
        const data = this._streamWrap(xmlString);
        elem = new DOMParser$1().parseFromString(data, 'text/xml').documentElement;
      }
      ...
      this._conn._dataRecv(elem, xmlString);
  ...
  _onOpen() {
    ...
    const data = encodeXmlString(startString);
